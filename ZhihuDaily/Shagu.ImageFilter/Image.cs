...
namespace Shagu.ImageFilter
{
    public class Image
    {
        ...
        {
            colorArray = new int[image.PixelBuffer.Length / 4];

            Buffer.BlockCopy(image.PixelBuffer.ToArray(), 0, colorArray, 0, colorArray.Length * 4);

            //var dd = new byte[] {1, 2, 3, 4, 5, 6, 7, 8};
            //var aa = new int[2];

            //Buffer.BlockCopy(dd, 0, aa, 0, 8);
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                {
                    int index = y * width + x;
                    var r = (colorArray[index] >> 16) & 0xff;
                    var g = (colorArray[index] >> 8) & 0xff;
                    var b = colorArray[index] & 0xff;
        ...
        { //从缓冲区中copy数据以加快像素处理速度
           ...
           // {
            ...
            var stream = destImage.PixelBuffer.AsStream();
            //Buffer.BlockCopy(colorArray, 0, destImage.PixelBuffer.ToArray(), 0, colorArray.Length * 4);
            byte[] data = new byte[colorArray.Length*4];
            Buffer.BlockCopy(colorArray, 0, data, 0, colorArray.Length*4);
        ...
        public static Image LoadImage(WriteableBitmap writeableBitmap) 
        {
            return new Image(writeableBitmap);
        }
