...
namespace Shagu.ImageFilter.Distort
{
    ...
    public class TwistFilter : BilinearDistort
    {
        double _twist;
        double _size;
        double _offsetX;
        ...
        public override double[] calc_undistorted_coord(int x, int y, double un_x, double un_y)
        {
            ...
            double u = x - width;
            double v = y - height;
            double r = System.Math.Sqrt(u * u + v * v);
            double theta = System.Math.Atan2(v, u);

            double t = 1 - ((r * _size) * invmaxrad);
            t = (t < 0) ? 0 : (t * t * t);
            theta += (t * _twist) / 100.0;

            un_x = Function.FClampDouble(width + r * System.Math.Cos(theta), 0.0, clone.getWidth() - 1.0);
            un_y = Function.FClampDouble(height + r * System.Math.Sin(theta), 0.0, clone.getHeight() - 1.0);
